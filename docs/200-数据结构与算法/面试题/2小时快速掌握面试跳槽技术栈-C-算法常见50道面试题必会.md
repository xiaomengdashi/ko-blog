---
sidebar_position: 1
slug: /数据结构与算法/面试题/2小时快速掌握面试跳槽技术栈-C-算法常见50道面试题必会
---

![](/img/posts/20f5dcfc57bbef4e1e433d2118505eef.gif)
...
![](/img/posts/386bcb5230439f40c4c3d7a8529a0b2d.png)

## 1. 1：链表反转
### 1.1. 【思路分析】**
链表反转的核心思想是改变链表节点之间的指针指向，将原本的正向连接变为反向连接。具体步骤如下：

+ 初始化三个指针：`prev`初始化为`nullptr`，用于记录前一个节点；`curr`初始化为链表的头节点，用于遍历链表；`next`用于临时保存当前节点的下一个节点。
+ 遍历链表，在遍历过程中，将当前节点的`next`指针指向前一个节点，然后更新`prev`、`curr`和`next`指针。
+ 当`curr`指针遍历到链表末尾（即`curr`为`nullptr`）时，反转完成，此时`prev`指针指向反转后链表的头节点。
...
![](/img/posts/ea2e348f74a249945ee66f653344ed90.png)

### 1.2. 【递归实现】**
除了迭代方法，还可以使用递归方法实现链表反转。递归方法的核心思想是先递归地反转后续节点，然后将当前节点的`next`节点的`next`指针指向当前节点，并将当前节点的`next`指针置为`nullptr`。
...
![](/img/posts/84dd706fb76255d7bf1b0ae7f827f96b.png)

### 1.3. 【递归复杂度分析】
+ **时间复杂度：****(O(n))****，其中 ****(n)**** 是链表的长度。需要递归遍历链表一次。**
+ **空间复杂度：****(O(n))****，主要是递归调用栈的空间开销。**

## 2. 2：如何判断一个单链表是有环的。
#### 2.0.1. 方法一：【哈希表法思路分析】**
使用一个哈希表（在C++中可以使用`std::unordered_set`）来记录已经访问过的节点。遍历链表，对于每个节点，检查它是否已经在哈希表中。如果在，则说明链表有环；如果不在，则将该节点加入哈希表，并继续遍历下一个节点。当遍历到链表末尾（即当前节点为`nullptr`）时，说明链表无环。
...
![](/img/posts/4bf54bc8abfe94c22f777a7db6da0198.png)

#### 2.0.2. 【复杂度分析】：**
+ **时间复杂度：****(O(n)\)****，其中 ****(n)****是链表的长度。需要遍历链表一次，对于每个节点，在哈希表中查找和插入操作的平均时间复杂度为****(O(1))****。**
+ **空间复杂度：****(O(n))****，主要用于存储哈希表中的节点，最坏情况下需要存储链表中的所有节点。**

### 2.1. 方法二：快慢指针法（Floyd判圈算法）思路分析**
使用两个指针，一个慢指针`slow`每次移动一步，一个快指针`fast`每次移动两步。如果链表有环，那么快指针最终会追上慢指针；如果链表无环，快指针会先到达链表末尾（即`fast`或`fast->`next`为`nullptr`）。
...
![](/img/posts/de22639b68d8efbc18b081a698f9d0b0.png)

#### 2.1.1. 【复杂度分析】**
+ **时间复杂度：****(O(n))****，其中(n)****是链表的长度。在最坏情况下，快指针和慢指针需要遍历链表一次。**
+ **空间复杂度：****(O(1))****，只使用了常数级的额外空间。**

## 3. 3：编写算法，从10亿个浮点数当中，选出其中最大的10000个 。
要从10亿个浮点数中选出最大的10000个，由于数据量巨大，无法一次性全部加载到内存中处理，因此可以采用外部排序的思想。以下是具体的实现步骤和C++代码：

### 3.1. 【实现步骤】**
1. **数据分割：将10亿个浮点数分割成多个小文件，每个小文件包含100万个浮点数。**
2. **局部排序：对每个小文件中的浮点数进行快速排序。**
3. **K 路合并：使用K路合并算法将排序好的小文件合并成一个大文件。**
4. **选取最大的10000个：从合并后的大文件中取出最大的10000个浮点数。**
...
### 3.2. 【代码解释】**
1. **快速排序：**`**quickSort**`**函数用于对每个小文件中的浮点数进行降序排序。**
2. **数据分割和局部排序：**`**splitAndSort**`**函数将输入文件分割成多个小文件，并对每个小文件中的浮点数进行快速排序。**
3. **K 路合并：**`**kWayMerge**`**函数使用最大堆来进行 K 路合并，从每个小文件中取出最大的元素，然后不断更新堆，直到取出最大的10000个元素。**
4. **主函数：在**`**main**`**函数中，首先调用**`**splitAndSort**`**函数进行数据分割和局部排序，然后计算小文件的数量，最后调用**`**kWayMerge**`**函数进行 K 路合并并输出最大的10000个元素。**

### 3.3. 复杂度分析
+ **时间复杂度：数据分割和局部排序的时间复杂度为****(O(nlogm))(O(nlogm))****，其中(n)**** 是总数据量，****(m)**** 是每个小文件的数据量。K路合并的时间复杂度为(O(klogk))****，其中(k)**** 是小文件的数量。因此，总的时间复杂度为(O(nlogm + klogk))****。**
+ **空间复杂度：主要的空间开销是每个小文件的排序和最大堆的使用，空间复杂度为(O(m + k))****。**

## 4. 4：最长回文子串
### 4.1. 方法一：【中心扩展法思路】**
回文串具有对称性，我们可以以每个字符或者每两个相邻字符为中心，向两边扩展来判断以该中心形成的回文子串的长度。遍历字符串中的每个可能的中心位置，找出最长的回文子串。
...
![](/img/posts/4e4a43d1d3d9376517485d39af149446.png)

### 4.2. 方法二：动态规划法思路**
定义一个二维布尔数组`dp[i][j]`表示字符串从索引`i`到索引`j`的子串是否为回文串。状态转移方程为：

![](/img/posts/fe0f84273f03985e8d9608b5a834264f.png)
...
![](/img/posts/88191fbb44120cd199275fd360ad8b0c.png)

### 4.3. 方法三：Manacher算法思路
Manacher算法是专门用于解决最长回文子串问题的高效算法，它利用了回文串的对称性，将时间复杂度优化到了(O(n))。具体做法是在原字符串的每个字符之间插入一个特殊字符（如`#`），将奇数长度和偶数长度的回文串统一处理，然后通过记录每个位置的回文半径来找出最长回文子串。
...
![](/img/posts/a41202cbad7d95293c5d8ada4ceadfe9.png)

#### 4.3.1. 【复杂度分析】
+ **时间复杂度：****(O(n))****，其中n是字符串的长度。每个字符最多被访问两次。**
+ **空间复杂度：(O(n))****，主要用于存储回文半径数组。**

中心扩展法和动态规划法实现相对简单，而Manacher算法时间复杂度最优，但实现较为复杂。
...
## 6. 6：-1,2,7,28,126请问28和126中间那个数是什么？为什么？
答 、应该是4^3-1=63  
规律是n^3-1(当n为偶数0，2，4)  
n^3+1(当n为奇数1，3，5)

## 7. 7：冒泡排序算法的时间复杂度是什么？o(n^2)。
![](/img/posts/f6dce60fc6378ce8887ac952b665b083.png)
...
![](/img/posts/a6d61439b211c42df654defe9d8c9293.png)
...
![](/img/posts/a276b0cbf61112be8ed50fadba058714.png)
...
### 8.1. 1：【活动选择问题】**
#### 8.1.1. 【问题描述】**
有n个活动，每个活动都有开始时间Si和结束时间Fi，要求选择一些活动，使得这些活动两两之间时间不冲突，且选择的活动数量最多。

#### 8.1.2. 【思路分析】**
按照活动的结束时间对所有活动进行排序，优先选择结束时间早的活动，这样可以为后续活动留出更多的时间。代码实现：
...
#### 8.1.3. 【复杂度分析】：**
+ **时间复杂度：排序的时间复杂度为 ****(O(n log n))****，遍历活动的时间复杂度为(O(n))****，因此总的时间复杂度为(O(n log n))****。**
+ **空间复杂度：主要用于存储排序后的活动，空间复杂度为(O(n))****。**

### 8.2. 2：【找零问题】**
#### 8.2.1. 【问题描述】**
给定不同面额的硬币`coins`和一个总金额`amount`，要求用最少数量的硬币凑出总金额`amount`，每种硬币的数量是无限的。

#### 8.2.2. 【思路分析】**
每次都选择面额最大的硬币，直到凑出总金额。

![](/img/posts/6adad297982dfdba2eef84f7179692e1.png)

#### 8.2.3. 复杂度分析**
**时间复杂度：排序的时间复杂度为：****  
**![](/img/posts/f756e21ca689144d293c15ef5c14103a.png)**  
**

其中 \(m\) 是硬币的种类数。遍历硬币和金额的时间复杂度为

![](/img/posts/ac772671edcdea5661ce9451a56a0960.png)**  
**

因此总的时间复杂度为

![](/img/posts/1867642c6cdd635a5850f534ed94c327.png)**  
**

**空间复杂度**：主要用于存储排序后的硬币，空间复杂度为 (O(m))。

### 8.3. 3：最小生成树（Kruskal算法）**
#### 8.3.1. 【问题描述】**
给定一个带权无向图，要求找出一棵连接所有顶点且边权之和最小的树。

#### 8.3.2. 【思路分析】**
将所有边按权值从小到大排序，然后依次选择边加入生成树中，只要加入该边不会形成环，直到生成树包含所有顶点。【代码实现】
...
![](/img/posts/14d810a2cea4f452a960ea2d3d8b473c.png)

#### 8.3.3. 复杂度分析**
+ **时间复杂度：排序边的时间复杂度为**

![](/img/posts/e556b744e3e4aeb84211bb5bb2be211b.png)

其中E是边的数量。并查集操作的时间复杂度接近

![](/img/posts/ab1e9e3d0f9a2af93cd0e4101660f233.png)

因此总的时间复杂度为

![](/img/posts/9c0f0eb90f4845812bdee260807cbe52.png)

**空间复杂度**：主要用于存储并查集的父节点数组，空间复杂度为

![](/img/posts/be465a264a9452b61e6c00f463a44fcb.png)

其中V是顶点的数量。

贪心算法在很多问题上可以高效地得到近似最优解，但并不是所有问题都适用，需要根据具体问题的性质来判断是否可以使用贪心算法。
...
答案：需要取模1e9+7（1000000007），如计算初始结果为：1000000008，请返回1。
...
![](/img/posts/858e07aed1bb21c36c94174431e51a0a.png)

## 10. 10：快速排序
快速排序（Quick Sort）是一种高效的排序算法，采用分治法（Divide and Conquer）策略。它的基本思想是通过选择一个基准元素（pivot），将数组分为两部分，使得左边部分的所有元素都小于等于基准元素，右边部分的所有元素都大于等于基准元素，然后分别对左右两部分递归地进行排序。以下是使用C++实现快速排序的代码：
...
![](/img/posts/f92adcc965980620e20fa46fb8aa464f.png)

**非递归形式下的快速快速排序：**

快速排序的非递归实现可以借助栈来模拟递归调用的过程。递归版本的快速排序是通过不断地递归调用自身来处理子数组，而非递归版本则使用栈来存储待处理的子数组的边界信息。以下是使用C++实现的非递归快速排序代码：
...
![](/img/posts/802fae59880948b4ba6ec942e2440570.png)

## 11. 10：冒泡排序
冒泡排序（Bubble Sort）是一种简单的排序算法，它重复地走访过要排序的数列，一次比较两个元素，如果它们的顺序错误就把它们交换过来。走访数列的工作是重复地进行直到没有再需要交换，也就是说该数列已经排序完成。以下是使用C++实现冒泡排序的代码：
...
![](/img/posts/79d96c71239ebfea4c2102244837d211.png)

## 12. 10：快速排序
快速排序（Quick Sort）是一种分治算法，它的基本思想是通过选择一个基准元素（pivot），将数组分为两部分，使得左边部分的所有元素都小于等于基准元素，右边部分的所有元素都大于等于基准元素，然后分别对左右两部分递归地进行排序。以下为你提供几种不同实现方式的快速排序C++代码。
...
![](/img/posts/4e783a72a1b43b1c4d5998bdeb5db4ba.png)
...
![](/img/posts/3aadbf6c1c89ebdd3757296da71aeb67.png)

## 13. 10：希尔排序
希尔排序（Shell Sort）是插入排序的一种改进版本，也被称为缩小增量排序。它的基本思想是将原始数据分成多个子序列来进行插入排序，随着增量逐渐减小，子序列的长度逐渐增加，整个序列会变得越来越接近有序，最后进行一次整体的插入排序，这样可以减少元素移动的次数，提高排序效率。以下是使用C++实现希尔排序的代码：
...
![](/img/posts/73eac7f5c537527a33907c308e6a7112.png)

## 14. 11：写一个在一个字符串(n)中寻找一个子串(m)第一个位置的函数。C++ KMP算法。
KMP（Knuth-Morris-Pratt）算法是一种用于字符串匹配的高效算法，其核心思想是利用已经匹配过的信息，避免在匹配过程中进行不必要的回溯，从而将时间复杂度优化到 (O(n + m))，其中n是主串的长度，m是子串的长度。

以下是使用C++实现KMP算法来在一个字符串中寻找子串第一个位置的代码：
...
![](/img/posts/8e7c4cac09612dae6e1d64a715e746a1.png)

## 15. 12：如何判断一个单链表是有环的？
判断一个单链表是否有环可以使用多种算法，下面主要介绍两种常见且高效的方法，分别是哈希表法和快慢指针法，并给出对应的C++代码实现。

### 15.1. 方法一：【哈希表法】思路**
使用一个哈希集合来记录已经访问过的链表节点。遍历链表时，检查当前节点是否已经在哈希集合中，如果存在，则说明链表有环；若遍历到链表末尾（即当前节点为`nullptr`）都未发现重复节点，则链表无环。代码实现：
...
## 16. 13：背包问题
#### 16.0.1. 【问题描述】**
给定一组物品，每个物品有对应的重量`w[i]`和价值`v[i]`，以及一个容量为`C`的背包。要求在不超过背包容量的前提下，选择一些物品放入背包，使得背包中物品的总价值最大，且每个物品只能选择放入或不放入背包（即0 - 1选择）。

#### 16.0.2. 【动态规划解法思路】**
使用二维数组`dp[i][j]`来表示前`i`个物品放入容量为`j`的背包中所能获得的最大价值。状态转移方程为：

![](/img/posts/a30953af4c598780c258e505c4c53cb3.png)
...
![](/img/posts/d38af1bca4ee89d47e893a655614c434.png)

**以下是使用DFS的C++实现：**

深度优先搜索（Depth-First Search，DFS）是一种用于遍历或搜索树或图的算法。该算法沿着树的深度遍历树的节点，尽可能深地搜索树的分支。当节点v的所在边都己被探寻过，搜索将回溯到发现节点v的那条边的起始节点。这一过程一直进行到已发现从源节点可达的所有节点为止。

下面分别给出在树和图上实现DFS的C++代码示例。树的DFS实现【问题描述】，给定一棵二叉树，对其进行深度优先搜索（前序遍历）。
...
![](/img/posts/4d7ab698ca6f1df0e0e1558091f179c9.png)

## 17. 15：动态规划
动态规划就是先取得当前情况下的最优解  成立就在原来的基础上加一 不成立就原地变成1。

### 17.1. 【最长递增子序列问题描述】**
给定一个无序的整数数组`nums`，找到其中最长递增子序列的长度。子序列是由数组派生而来的序列，删除（或不删除）数组中的元素而不改变其余元素的顺序。例如，`[3,6,2,7]` 是数组 `[0,3,1,6,2,2,7]` 的一个子序列。

### 17.2. 【动态规划思路】**
+ **定义状态：设**`**dp[i]**`**表示以第**`**i**`**个元素结尾的最长递增子序列的长度。**
+ **初始化：每个元素自身可以构成一个长度为1的递增子序列，所以初始时**`**dp[i] = 1**`**，其中**`**i**`**从0到 **`**nums.size() - 1**`**。**
+ **状态转移方程：对于每个**`**i**`**，遍历**`**0**`**到**`**i - 1**`**的所有 **`**j**`**，如果**`**nums[j] < nums[i]**`**，说明可以将第**`**i**`**个元素接在以第**`**j**`**个元素结尾的递增子序列后面，此时更新**`**dp[i] = max(dp[i], dp[j] + 1)**`**。**
+ **最终结果：最长递增子序列的长度为**`**dp**`**数组中的最大值。**
...
![](/img/posts/5fabceca025376d8e459b33db473053e.png)

## 18. 16：树的两种遍历得到树并按照第三种遍历方式输出。
在树的遍历中，常见的遍历方式有前序遍历（根 - 左 - 右）、中序遍历（左 - 根 - 右）和后序遍历（左 - 右 - 根）。已知其中两种遍历序列，通常可以唯一确定一棵树（前提是包含中序遍历序列，因为中序遍历可以区分左右子树），然后按照第三种遍历方式输出。下面以已知前序遍历和中序遍历序列，构建树并进行后序遍历输出为例，给出C++代码实现。

### 18.1. 思路分析**
1. **前序遍历特点：前序遍历的第一个元素是根节点。**
2. **中序遍历特点：根节点将中序遍历序列分为左子树和右子树两部分。**
3. **构建树的过程：**
    - 从前序遍历序列中取出第一个元素作为根节点。
    - 在中序遍历序列中找到该根节点的位置，以此将中序遍历序列分为左子树和右子树。
    - 根据左子树和右子树的长度，从前序遍历序列中划分出左子树和右子树的前序遍历序列。
    - 递归地构建左子树和右子树。
4. **后序遍历输出：按照左 - 右 - 根的顺序遍历构建好的树。**
...
![](/img/posts/2824e196a3a2505205ac9c06d0c40d83.png)

## 19. 17：unordered_map计统计string中出现的char的次数。
在C++中，可以使用`std::unordered_map`来统计字符串中每个字符出现的次数。`std::unordered_map`是一种关联容器，它存储键值对，并且可以通过键快速访问对应的值，非常适合用于字符计数这种场景。以下是具体的代码实现：
...
![](/img/posts/15dc05aceb0a9c035a02e6278e690c25.png)

## 20. 18：string中的最长的不重复子串（双指针+unordered_map）。
### 20.1. 算法思路**
1. **双指针：使用两个指针**`**left**`**和**`**right**`**来维护一个滑动窗口，**`**right**`** 指针不断向右移动，用于扩展窗口；**`**left**`**指针根据情况移动，用于收缩窗口，以保证窗口内的子串不包含重复字符。**
2. `**unordered_map：使用**``**`unordered_map&lt;char, int>`**`**来记录每个字符最后一次出现的位置。当**`**right**`**指针遇到一个已经在 **`**unordered_map**`** 中存在的字符时，需要检查该字符上次出现的位置是否在当前窗口内。如果在窗口内，需要移动**`**left**`**指针到该字符上次出现位置的下一个位置，以保证窗口内的子串不包含重复字符。**
3. **记录最长子串长度：在移动指针的过程中，不断更新最长不重复子串的长度。**
...
![](/img/posts/a2d514ff0157618613e0551ca94d09d6.png)
...
**本质上就是swap和迭代 直到nullptr然后return。**
...
最长的连续字符子串  区别在于else后面的结果直接就是0不需要再比较大小。
...
把string存入set然后对每一个元素的子元素字符串（后面开始） 删除  最后的结果就是set的每一个元素的长度+1的和string。substr(i)  从i开始到最后的字符串就是答案。
...
**这个是简单版本 容易理解的版本  这是把所有的数据全部写入进去 然后取其中的一个值  定义abc时 需要longlong因为虽然是int但是乘积的时候会超出 所以需要用long long。**
...
**32：****删除链表的倒数第k个结点在这个基础上还可以输出倒数某一个节点的值。**
...
## 34. 33：旋转链表。
**首先是k%len得到一个数，因为len就是一个循环，然后对于余数，可以在链表的len-余数的位置使得node->next = nullptr;**

## 35. 34：两个字符串的最长公共子串----动态规划。
**这是个动态规划的题动态规划的关键在于构建一个二维数组。**
...
**背包问题：行成m+1 n+1矩阵 外围为0然后对于每一个i比较第i行的数据  然后j变化  第一个出现的数  可以认为是  在i个物体的情况下，权重w为j的情况下，即前i个物体中，最大值放在了第一个，然后依次类推，放两个放3个放i个物体。**
