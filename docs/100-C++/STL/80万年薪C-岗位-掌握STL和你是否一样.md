---
sidebar_position: 1
slug: /C++/STL/80万年薪C-岗位-掌握STL和你是否一样
---

![](/img/posts/50150ffe1bf7f471cf9bb024a7e55fe1.png)
...
+ 获取容器大小：通过 `size()` 成员函数可以获取容器中当前元素的个数，例如 `vector`、`list`、`set` 等容器都支持该操作，像 `std::vector<int> v; v.size();` 就能得到 `v` 的元素个数。

+ 判断容器是否为空：使用 `empty()` 成员函数，若容器为空返回 `true`，否则返回 `false`，如 `std::stack<int> st; st.empty();` 判断栈是否为空。
