...
```
+ **参数**：`F&& f` - 可调用对象，接受 `T` 类型参数，返回 `std::optional<U>` 类型
+ **返回值**：函数返回的 `std::optional<U>` 类型
+ **功能**：对包含的值应用返回 `optional` 的函数，实现条件性链式操作
+ **调用案例**：
  ```cpp
...
```
+ **说明**：只有当值大于 0 时才保留，否则返回空 `optional`
...
+ **功能**：当 `optional` 为空时提供替代值，有值时直接返回原值
+ **调用案例**：
  ```cpp
  auto fallback = empty_opt.or_else([]() {
...
constexpr auto and_then(F&& f) && -> std::invoke_result_t<F, T&&>;
```
+ **参数**：`F&& f` - 可调用对象，接受 `T` 类型参数，返回 `std::expected<U, E>` 类型
+ **返回值**：函数返回的 `std::expected<U, E>` 类型
+ **功能**：对成功值应用返回 `expected` 的函数，可以引入新的错误
+ **调用案例**：
  ```cpp
  auto result = exp.and_then([](int x) -> std::expected<int, std::string> {
      return x > 0 ? x : std::unexpected("Invalid");
  });
```
+ **说明**：只有当值大于 0 时才成功，否则返回错误
...
```
+ **参数**：`F&& f` - 可调用对象，接受 `E` 类型参数，返回 `std::expected<T, F>` 类型
+ **返回值**：函数返回的 `std::expected<T, F>` 类型
+ **功能**：错误时提供恢复逻辑，成功值直接传播
+ **调用案例**：
  ```cpp
  auto recovered = error_exp.or_else([](const std::string& err) -> std::expected<int, std::string> {
      return 42;
  });
```
+ **说明**：当包含错误时，返回默认值 42 作为恢复
...
+ **返回值**：`std::expected<T, F>`，其中 `F` 是错误变换函数的返回类型
