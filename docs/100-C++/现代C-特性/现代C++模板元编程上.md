...
// 只接受算术类型的模板
template <typename T,
          typename = typenamestd::enable_if<std::is_arithmetic<T>::value>::type>
T square(T value) {
    return value * value;
}
```
...
```
`std::enable_if<条件, 类型>::type` 在条件为 true 时等于指定的类型，条件为 false 时不提供 `type` 成员，从而触发 SFINAE。
...
```cpp
template <typename T,
          typename = typenamestd::enable_if<std::is_integral<T>::value>::type>
void foo(T t) { /*...*/ }
```
3. **函数参数**：
