---
sidebar_position: 1
slug: /C++/现代C++现代特性/现代C++的内存管理
---

## 1. 智能指针之所以智能的核心原理：RAII机制
...
#### 3.2.2. 2.构造函数（默认是传入已经实例化好的对象，构造函数只实例化控制块）
```cpp
// 1. 初始化原始指针成员
...
##### 3.2.5.1. 强引用计数归零（资源销毁）**
当最后一个 `shared_ptr` 离开作用域或被重置时，触发以下步骤：

###### 3.2.5.1.1. 1）析构函数调用 `__release_shared()`
 中 `shared_ptr` 析构函数实现：
