...
+ `std::tuple<T...>` 是存储 `T...` 类型对应的各自值（一个类型对一个值）
+ `std::variant<T...>` 可以存储 `T...` 这些类型中的一种类型的一个值（允许多种类型，但实际只存储其中一种类型的一个值）。
+ std::any可以存储任何类型的一个值
...
struct Visit {
  ...
  void operator()(conststd::vector<int>&v) {
    std::cout << "vector: ";
    for (auto &i : v) {
      std::cout << i << " ";
    }
...
struct Visit_Return {
  ...
  std::string operator()(conststd::vector<int>&v) {
