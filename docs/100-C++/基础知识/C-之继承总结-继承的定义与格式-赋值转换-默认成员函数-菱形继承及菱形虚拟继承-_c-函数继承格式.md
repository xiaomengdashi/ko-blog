---
sidebar_position: 10
slug: /C++/基础知识/C-之继承总结-继承的定义与格式-赋值转换-默认成员函数-菱形继承及菱形虚拟继承-_c-函数继承格式
---
...
+ 运行结果：  
![](/img/posts/63042621260f6077324aca810da723af.png)
+ 总结：继承后父类的Person的成员（成员函数+成员变量）都会变成子类的一部分。这里体现出了Student和Teacher复用了Person的成员。

### 1.2. 1.2继承的定义
#### 1.2.1. 1.2.1定义格式
![](/img/posts/46b8fd32aacee9d2ded55af94c03fc00.png)  
![](/img/posts/3dca21e29f5a426afbc788312491af7b.png)

#### 1.2.2. 1.2.2继承关系和访问限定符
![](/img/posts/9b4d20388c8ebc3988092630524552e5.png)

#### 1.2.3. 1.2.3继承[基类](https://so.csdn.net/so/search?q=%E5%9F%BA%E7%B1%BB&spm=1001.2101.3001.7020)成员访问方式的变化
![](/img/posts/ec789d34a949c31779952d2856209d6a.png)
...
![](/img/posts/1900b56eccc3702f3e5b207fbb8d3410.png)
+ **我们对应看三个例子：**
+ **赋值给基类的对象**

![](/img/posts/81c66b27fabf9e3aaa8f52bc58dac09e.png)

+ **赋值给基类的引用**  
![](/img/posts/c63f571a3205a9b84464329d9ec5fd48.png)
+ **赋值给基类的指针**  
![](/img/posts/52c534000d39bca1ba378f3744d0cdf3.png)
+ **总结**：  
![](/img/posts/c2a7d05178e487faff1b65e9d3b71c1b.png)

## 3. 3.继承中的[作用域](https://so.csdn.net/so/search?q=%E4%BD%9C%E7%94%A8%E5%9F%9F&spm=1001.2101.3001.7020)
...
+ **举例：**  
![](/img/posts/17d0d0d3cc729864c66907902fe507d4.png)

## 4. 4.派生类的默认成员函数
+ **6个默认成员函数**，“默认”的意思就是指我们不写，编译器会变我们自动生成一个，那么在派生类中，**这几个成员函数是如何生成的呢？**
+ `Ⅰ`.**构造函数**：派生类的构造函数**必须调用基类的构造函数初始化基类的那一部分成员**。如果基类没有默认的构造函数，则必须在派生类构造函数的初始化列表阶段显示调用  
![](/img/posts/bd48406e5b6a3a5feda245ca162d3783.png)
+ `Ⅱ`.**拷贝构造**： 派生类的拷贝构造函数必**须调用基类的拷贝构造完成基类的拷贝初始化**  
![](/img/posts/c443ed8febacb9348259f6935538204c.png)
+ `Ⅲ`. **赋值运算符重载**：`派生类的operator=`必须要`调用基类的operator=`完成基类的赋值  
![](/img/posts/ee50f62608119a41138a133b6f6d0cfa.png)
+ `Ⅳ`.**派生类的析构函数**会在被调用完成后**自动调用基类的析构函数清理基类成员**。因为这样才能保证派生类对象**先清理派生类**成员**再清理基类**成员的顺序（如果自己显式调用，会存在父类先析构问题）  
![](/img/posts/9023c008337214e711de02d24e6e80eb.png)  
![](/img/posts/8ccd150ac45b3d2f57b394acd31874d1.png)
+ `Ⅴ`.派生类对象初始化先调用基类构造再调派生类构造
+ `Ⅵ`.派生类对象析构清理**先调用派生类**析构**再调基类的析构**  
![](/img/posts/b47838a67720a54025e737b3795cb7c4.png)

## 5. 5.继承与友元
+ **友元关系不能继承**，也就是说**基类友元不能访问子类私有和保护成员**  
![](/img/posts/df02e410cc858ff245521dd0a44b1651.png)

## 6. 6.继承与[静态成员](https://so.csdn.net/so/search?q=%E9%9D%99%E6%80%81%E6%88%90%E5%91%98&spm=1001.2101.3001.7020)
+ **基类定义了static静态成员，则整个继承体系里面只有一个这样的成员**。无论派生出多少个子类，都`只有一个static成员实例`  
![](/img/posts/071c0b676c21905ba013cb2088bfa733.png)

## 7. 7.菱形继承及菱形虚拟继承
### 7.1. 7.1单继承与多继承
### 7.2. 7.2菱形继承存在的问题及解决方案
![](/img/posts/8e57f98d1c1b6d53c9fb8eb6a2f38af3.png)
...
![](/img/posts/fdebf25d4526e5d346bacc68729fd965.png)

### 7.4. 7.4虚拟继承解决数据冗余和二义性的原理
+ 先分析菱形继承

![](/img/posts/8b41e233a71006461f17dc8664c1975c.png)

+ 再分析菱形虚拟继承

![](/img/posts/b4e9958ef2522e50fecbc5f2f317495e.png)
...
![](/img/posts/e59f944256b98bbb68f3de8c29624688.png)
+ 组合是`has-a`，因为对象的内部细节是不可见的。对象只以“黑箱”的形式出现。 组合类之间没有很强的依赖关系，耦合度低。优先使用对象组合有助于你保持每个类被封装  
![](/img/posts/801230a11369054ab92c9c0e36c5f6bd.png)
+ 优先使用对象组合，而不是类继承
+ 继承和组合都是类设计角度的复用
