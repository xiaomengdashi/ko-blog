...
+ `addTask(std::function<void()> f)`：将任务添加到任务队列，并通知一个线程有新任务到来。

  - 线程的任务函数是一个可调用对象，类型为 `std::function<void()>`，返回值类型为 `void`，无参数。
  - 对应的任务队列为 `std::queue<std::function<void()>> m_tasks`
...
+ `auto` 返回类型：
  ...
  - 返回类型是 `std::future<typename std::result_of<F(Args...)>::type>`，表示 `addTask` 函数会返回一个 `future` 对象，用于异步获取任务的结果。`std::result_of<F(Args...)>::type` 用于推导 `F` 运行后的返回类型。
  - 通过使用 `typename`，我们明确告诉编译器 `std::result_of<F(Args...)>::type` 是一个类型，而不是其他实体（例如静态成员）。

+ `using returnType`：

  - 使用 `std::result_of<F(Args...)>::type` 来获取任务函数 `F` 执行后的返回类型，并将其命名为 `returnType`。
...
+ **任务队列**：
  ...
  - `std::unique_lock<std::mutex> lock(m_queueMutex)` 用于加锁，确保线程安全地将任务加入任务队列。
