---
sidebar_position: 1
slug: /C++/项目实战/从零实现Malloc：sbrk与mmap如何选择？
---

🤔 为什么要从零实现malloc？这不仅是一个经典的系统编程面试题，更是深入理解计算机系统的绝佳机会！通过实现自己的内存分配器，你将掌握：

🎯 核心收获：

+ 🔧 sbrk和mmap系统调用的底层原理
+ ⚡️ 内存分配器的基础设计思路
+ 🐛 program break和内存布局的深入理解

📚 本教程将带你踏上一段激动人心的探索之旅！我们将：
...
2. 通过`template<size_t Alignment>`实现策略模式
3. 利用`std::unique_ptr`自定义删除器管理内存块
4. 使用`std::atomic`实现无锁链表操作

保持原有代码实现不变，通过后续阶段逐步添加功能模块
...
接下来的章节，我们将逐步完善这些缺失的功能。💪✨

### **🔍**** 深入理解sbrk系统调用**
🖥️✨ sbrk是Unix/Linux系统中管理堆空间的核心系统调用💎，通过移动"program break"🎯（堆内存的上边界📏）来动态调整进程的数据段大小📊。其核心操作包括🌟：

+ 📈 扩展内存：`sbrk(正数N)`🔄 返回旧地址并扩展N字节堆空间🚀
+ 📉 收缩内存：`sbrk(-N)`⏬ 可回收已分配的堆空间♻️
+ 🔎 查询地址：`sbrk(0)`📍 获取当前堆顶位置而不改变内存布局🕵️♂️

📝🎨 根据内存布局图和sbrk工作原理，我们通过图形展示内存变化过程🌈：
...
2. ⚡️🔥 每次sbrk(N>0)调用：
    - 🔼📈 堆空间向上增长（地址增大）🚀
    - 🎯📤 返回旧program break值作为分配地址💡
3. 🔽⏬ sbrk(N<0)时：
    - ⬇️📉 program break下移（地址减小）🔽
    - ⚠️🚫 但不会主动合并或清除元数据🧹
4. 📝📦 元数据头始终位于分配块起始位置（通过header+1返回用户空间）👨💻
...
### **🔍**** 深入理解 mmap 系统调用 ****🚀**
除了sbrk 🔧，mmap 🗺️ 也是一个重要的内存分配系统调用 💫。让我们来对比这两种方式 ⚖️：
...
### **为什么不直接使用 mmap 来获取内存呢？**
直接使用 mmap 虽然方便，但存在一些明显的局限性 🤔：
...
2. 🔸 频繁分配的小内存 → 选择sbrk+空闲链表 🔄⛓️
3. 🎯 中间释放的sbrk内存 → 链表实现高效复用 ♻️💡
4. ⚡️ 性能关键场景 → 混合使用策略 🎭🔀

### **🎯**** 本章总结与展望**
🌟 本章核心收获：

+ ✅ 掌握了sbrk/mmap系统调用的底层原理
+ 🔧 理解了原始内存分配方式的局限性
+ 📊 对比了不同内存管理策略的优劣
+ 🚀 实现了最简内存分配器原型
...
准备好迎接更深入的内存管理挑战了吗？让我们在下一章共同打造真正的内存管理系统！ 💪🚀

### **🔥🚀**** C++20/17 精华手册速递**
🎉🚀 C++20 性能怪兽：协程异步起飞🔄✈️ + 模板概念零崩溃📦🛡️ + 太空船运算符爽到飞起🚀👨🚀！  
🛠️💎 C++17 生存利器：结构化绑定拆箱即用🎁📦 + 并行算法榨干CPU⚡🔥 + 文件系统操作比发朋友圈还简单📁🤳！

🛎️📬 获取方式： ▸ 回复【17】📩👉领《C++17生存指南》📘🔖 ▸ 回复【20】📩👉拿《C++20进阶秘籍》🔧🚀
...
> 来自: [从零实现Malloc：sbrk与mmap如何选择？](https://mp.weixin.qq.com/s/kOYIHayYFWkhSj9yiqW66w)
>
